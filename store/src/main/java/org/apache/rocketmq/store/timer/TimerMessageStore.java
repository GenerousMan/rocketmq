...
public class TimerMessageStore {
    ...
    public TimerMessageStore(final MessageStore messageStore, final MessageStoreConfig storeConfig,
                             TimerCheckpoint timerCheckpoint, TimerMetrics timerMetrics, final BrokerStatsManager brokerStatsManager) throws IOException {
            ...
            while (maxCount-- > 0) {
                ...
                try {
                    ...
                    if (offsetPyTemp == offsetPy && sizePyTemp == sizePy) {
                        log.info("reviseQueueOffset check cq offset ok. {}, {}, {}",
                    ...
                    log.error("reviseQueueOffset check cq offset error.", e);
                } finally {
                    bufferCQ.release();
                }
            }
    ...
    class TickService extends ServiceThread {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                try {
                    timerWheel.Tick(System.currentTimeMillis());
                    waitForRunning(precisionMs);
                    continue;
                } catch (Throwable e) {
                    TimerMessageStore.log.error("Unknown error", e);
                }
            ...
            while (!this.isStopped()) {
                try {
                    if (System.currentTimeMillis() < shouldStartTime) {
                        TimerMessageStore.log.info("TimerDequeueGetService ready to run after {}.", shouldStartTime);
                        waitForRunning(1000);
                        continue;
                    }
